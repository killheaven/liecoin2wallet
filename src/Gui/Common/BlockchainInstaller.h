...
private:
  ...
  const QDir m_LiecoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfLiecoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _Liecoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo LiecoinBlockchainInfo() const;
