...
const char DEFAULT_WALLET_FILE_NAME[] = "Liecoin.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "172.104.109.14:4444";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("Liecoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("Liecoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("Liecoinwallet.desktop");
  ...
  res = autorunSettings.contains("LiecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("LiecoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.Liecoin.Liecoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Liecoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("LiecoinWallet", appPath);
    } else {
      autorunSettings.remove("LiecoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\Liecoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:Liecoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\Liecoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\Liecoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("Liecoinwallet.cfg"));
