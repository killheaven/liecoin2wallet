...
const char BYTECOIN_URI_SCHEME_NAME[] = "Liecoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("Liecoinwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("Liecoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Liecoin wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Liecoin wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have Liecoind with non-default RPC port, you should terminate it and relaunch LiecoinWallet\n"
      "or\n"
      "Set the Local deamon required port in LiecoinWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
